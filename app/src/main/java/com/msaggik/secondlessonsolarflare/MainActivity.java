...
public class MainActivity extends AppCompatActivity {
    ...
    private int[] solarFlare = {60, 70, 85}; // Время солнечной вспышки
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        output.setText(coreTime(delayMinute, volumeData, coreFrequency, solarFlare) + " секунд");
    }
    ...
    // МОДУЛЬ 2
    // метод определения времени обработки компьютером данных с одной планеты в секундах (скорость компьютера, размер данных, солнечная вспышка)
    private int volumeTime(int coreFrequency, int volumeData, int solarFlare) {
        return volumeData * solarFlare / coreFrequency / 60;
    }

    // МОДУЛЬ 3 (использующий МОДУЛИ 1-2)
    // метод вычисления времени обработки компьютером приходящей информации в секундах (массив задержек, массив объёмов данных, скорость компьютера, солнечная вспышка)
    private int coreTime(int[] delayMinute, int[] volumeData, int frequency, int[] solarFlare) {
        ...
        for (int i = 0; i < volume.length; i++) { // инициализация данного массива
            volume[i] = volumeTime(frequency, volumeData[i], solarFlare[i]); // определение времени обработки компьютером данных от одного спутника
        }
